                    ...
                    style="width: 100%">
                        ...
                        <template slot-scope="scope">
                            <el-button
                              size="mini"
                              type="warning"
                              @click="searchArticle(scope.$index, scope.row)">导出</el-button>
                            <el-button
                                size="mini"
                                type="danger"
                                @click="deleteBaiduUrl(scope.$index, scope.row)">删除</el-button>
                          </template>
                      </el-table-column>
                    </el-table>
                    <el-pagination
...
<script>
    export default {
        ...
        methods: {
            ...
            //导出zip
            searchArticle(index, row){
                axios.get('/baidu_urls/'+row.id+'/search')
               .then((response)=> {
            ...
            //删除
            deleteBaiduUrl(index, row){
                axios.delete('/baidu_urls/'+row.id)
               .then((response)=> {
                    console.log(response);
                    let message = {};
                    if(response.data.status == 200){
           ...
           getPage(page){
               //加载table数据
               page = page || 1
               axios.get('/baidu_urls?page='+page)
               .then((response)=> {
                   console.log(response)
